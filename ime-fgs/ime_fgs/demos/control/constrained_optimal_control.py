...
from ime_fgs.divergence_measures import mode_matched_mean_cov_of_doubly_truncated_gaussian
...
if __name__ == '__main__':
    ...
    seed(1)
    z_real = 1 * concatenate([linspace(0, 0, 1 * num_meas // 5),
                              linspace(2, 2, 3 * num_meas // 5), linspace(0, 0, 1 * num_meas // 5)])
    ...
    x0 = np.atleast_2d(0.)
    initial_state = GaussianMeanCovMessage(mean=x0, cov=[[1e-8]])
    ...
    # plt.plot(squeeze([fwd_estimate.mean for fwd_estimate in lqg.get_state_out_msgs()]), 'b-',
    ...
    with open('TikZ-constrainedOptimalControlEP.dat', 'w', newline='') as csvfile:
        fieldnames = ['t', 'y', 'u', 'r']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter='\t')
        writer.writeheader()
        for ii in range(len(z))[0::4]:
            writer.writerow({fieldnames[0]: str(ii), fieldnames[1]: str(squeeze(y_fg[ii])),

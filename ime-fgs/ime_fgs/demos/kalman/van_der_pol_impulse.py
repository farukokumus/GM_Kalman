...
def setup_example():
    ...
    n_meas = 240
    inputs = np.zeros((1, n_meas))
    inputs[:, [55]] = 2.0
    inputs[:, [115]] = -2.0

    return KalmanExample(A=A, B=B, C=C, initial_state=x0, input_noise_cov=0,
                         process_noise_cov=[[0.00005, 0, 0], [0, 0.00005, 0], [0, 0, 0.0000000005]],
...
def generate_paper_plots():
    ...
    #                                    do_backward=True, input_estimation_cov=10)
    ...
    kf_obj = example.run_kf_on_example(KalmanFilter(slice_type=KalmanSlice,
                                       ...
                                       do_backward=True, input_estimation_cov=1, num_iterations=10)
    ...
    ref_sigs = {}
    # Input signals
    df_to_plot['u (t)'] = example.sim_result.inputs[0, :-n_samples_to_omit]
    ref_sigs['u (t)'] = pd.DataFrame({'Estimate': np.zeros((len(example.sim_result.time) - n_samples_to_omit,))})
    ref_sigs['u (t)'].loc[1:, 'Estimate'] = \
        GaussianMeanCovMessage.get_means(input_estimation_results)[0, :-n_samples_to_omit]
    ref_sigs['u (t)'].loc[0, 'Estimate'] = np.nan
